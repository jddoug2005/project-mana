...
var leash_range: float = 300.0 # Max distance from scout_center before returning

# --- Logic Variables ---
var current_path: PackedVector2Array = []
var HALF_TILE = Vector2(Global.TILE_SIZE / 2.0, Global.TILE_SIZE / 2.0)
var is_selected = false
var scout_center: Vector2 # The point the unit "guards"
var target_enemy: Node2D = null
var is_persistent_chase: bool = false # True if player ordered the attack
...
func find_path(target_pos: Vector2, astar: AStarGrid2D, persistent_target: Node2D = null):
	# If player clicked an enemy specifically
	if persistent_target:
		target_enemy = persistent_target
		is_persistent_chase = true
		current_state = State.CHASING
	else:
		# Player clicked ground: Move there and make it the new guard post
...
func check_for_enemies():
	# Scan for anyone in the "enemies" group
...
func handle_chase_logic():
	...
	# LEASHING: Give up if too far from guard post, unless player specifically ordered the kill
	if not is_persistent_chase and dist_to_center > leash_range:
		return_to_scout_center()
		return

	# ATTACKING: Stop and fight if in range
...
func return_to_scout_center():
	...
	# Request pathfinding from World via group or local calculation
	# For now, we'll set state to returning; World.gd will handle actual pathing on next click
	# but we'll force a direct move for this simple logic
